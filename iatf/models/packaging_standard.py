...
class PackagingStandard(models.Model):
    ...
    supplier_type = fields.Selection([('supplier', 'Supplier'), ('customer', 'Customer')], default='supplier',
                                     string='Type')
    # supplier_id = fields.Many2one('res.partner', 'Supplier Name/ Customer name')
    ...
    secondary_separator_details = fields.Char('Separator ate=True')
    ...
    # approve_department_ids = fields.Many2many('hr.department', 'ta3', string='Departments Approvals')
    ...
    # manager_ids = fields.Many2many('hr.employee', 'ta4', string='Approval Managers')
    ...
    #     action_id = self.env.ref('iatf.action_iatf_packing_standard_window', raise_if_not_found=False)
    ...
    def action_generate_report(self):
        ...
        for rec in self:
            ...
            ws['F2'] = rec.project_id.name if rec.project_id else ''
            ws['I2'] = rec.partner_id.name if rec.partner_id else ''
            ws['F3'] = rec.part_name if rec.part_name else ''
            ...
            ws[
                'G17'] = rec.final_detail_steel_nylon_other_clamping if rec.final_detail_steel_nylon_other_clamping else ''
        ...
        cur_row = 25  # As per Sheet in current row should be added
