...
class ToolsTrackingSheet(models.Model):
    _name = 'tools.trackingsheet'
    _description = 'Tools Tracking Sheet'
    ...
    # part_no = fields.Char("Part No.", related="part_name.default_code", store=True)True
    ...
    line_ids = fields.One2many('tools.trackingsheet.line', 'tracking_id', string="Inspection Lines")
    ...
    def create(self, vals_list):
        records = super(ToolsTrackingSheet, self).create(vals_list)
        ...
        for record in records:
            ...
            for operation in pm_records.process_presentation_ids.sorted(key="operation"):
                for tools_element in operation.operation_lines_ids:
                    for i in tools_element.tool_ids:
                        
                        self.env['tools.trackingsheet.line'].create({
                            'tracking_id': record.id,
                            'element_no': tools_element.element_no,
    ...
    def action_update_control_plan(self):
        ...
        for control_plan in control_plan_recs:
            # Retrieve the approved tools Tracking Sheet for the project
            tool_sheet = self.env['tools.trackingsheet'].search([
                ('project_id', '=', self.project_id.id),
                ('final_status', '=', 'approved')
            ])
            if not tool_sheet:
                raise ValidationError(_("No approved tools Tracking Sheet found for this project."))

            # Group tools lines by operation number
            operation_groups = {}
            for tool_line in tool_sheet.line_ids:
                if tool_line.operation_no not in operation_groups:
                    operation_groups[tool_line.operation_no] = {
                        'description': tool_line.description,
                        'tools_lines': []
                    }
                operation_groups[tool_line.operation_no]['tools_lines'].append(tool_line)
            ...
            for operation_no, operation_data in sorted(operation_groups.items()):
                ...
                # Create characteristic lines for each piece of tools
                for tool_line in operation_data['tools_lines']:
                    # Check if this tool is already added to this process
                    existing_char_line = self.env['control.chara.line'].search([
                        ('chara_id', '=', process_line.id),
                        ('char_no', '=', tool_line.element_no)
                    ], limit=1)

                    if existing_char_line:
                        # Append new tool(s) to the existing characteristic line
                        existing_char_line.mc_jig_tool = [
                            (6, 0, list(set(existing_char_line.mc_jig_tool.ids + [tool_line.tool_name.id])))]
                        existing_char_line.mc_jig_tool_num = ', '.join(
                            filter(None, [existing_char_line.mc_jig_tool_num, tool_line.tool_no])
                        )
                    else:
                        self.env['control.chara.line'].create({
                            'chara_id': process_line.id,
                            'char_no': tool_line.element_no,
                            'mc_jig_tool': [(6, 0, [tool_line.tool_name.id])],
                            'mc_jig_tool_num': tool_line.tool_no,
                        })
        ...
        return {
            ...
            'params': {
                'title': _("Success"),
                'message': _("Control Plan updated successfully with tools data!"),
                'sticky': False,
                'type': 'success',
            }
        }
    ...
    def generate_xls_report(self):
        ...
        col_widths = {
            ...
            'K': 15, 'L': 15, 'M': 15, 'N': 15, 'O': 15, 'P': 15, 'Q': 18, 'R': 20
        }
        ...
        for rec in self:
            ...
            for ele in rec.line_ids:
                ws[f'A{row}'] = ele.tool_name.name if ele.tool_name.name else ''
                ws[f'B{row}'] = ele.tool_no if ele.tool_no else ''
                ws[f'C{row}'] = ele.operation_no if ele.operation_no else ''
                ws[f'D{row}'] = ele.description if ele.description else ''
                ws[f'E{row}'] = ele.availability if ele.availability else ''
                ws[f'F{row}'] = ele.machine_name if ele.machine_name else ''
        ...
        attachment = self.env['ir.attachment'].create({
            'name': 'Tools_Tracking_Sheet.xlsx',
            'type': 'binary',
...
class ToolsTrackingSheetLine(models.Model):
    _name = 'tools.trackingsheet.line'
    _description = 'Tools Tracking Sheet Line'
    _inherit = "translation.mixin"

    tracking_id = fields.Many2one('tools.trackingsheet', string='Tools Tracking Sheet',
                                  ondelete="cascade")

    
    element_no = fields.Char(string="Element No")
    tool_name  = fields.Many2one("maintenance.equipment",string="Tool Name", domain="[('category_id', '=', 'Tool' )]")
    tool_no = fields.Char(related='tool_name.serial_no',string="Tool No.")
    ...
    machine_name = fields.Many2one('maintenance.equipment', string="Machine Name")
