...
class PfmeaDashboardDetails(models.Model):
    _name = 'pfmea.dashboard.details'
    _description = 'PFMEA Dashboard Details'
    ...
    failure_mode = fields.Char(string='Failure Mode', readonly=True)
    failure_causes = fields.Char(string='Failure Cause', readonly=True)
    fmea_ap = fields.Char(string='Risk Analysis Action Priority', readonly=True)
    pfmea_ap = fields.Char(string='Optimization Action Priority', readonly=True)
    ...
    def refresh_top_ap_data(self):
        ...
        self.env['pfmea.dashboard.details'].search([]).unlink()
...
class PfmeaScheduler(models.Model):
    _name = 'pfmea.scheduler'
    _description = 'Top AP for PFMEA Scheduler'

    @api.model
    def refresh_top_ap(self):
        self.env['pfmea.dashboard.details'].refresh_top_ap_data()
        return True
