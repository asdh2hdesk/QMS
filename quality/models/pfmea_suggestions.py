...
class PfmeaSuggestions(models.Model):
    _name = 'pfmea.suggestions'
    _description = 'Pfmea Suggestions'
    ...
    process_item = fields.Char(string='Name of Process',translate=True)
    ...
    # Failure Effects
    fe_in_plant = fields.Char(string='In Plant',translate=True)
    ...
    func_of_process_step = fields.Many2one('process.characteristics',
                                           string='Function of Process Step and Process Characteristics')
    func_of_product_step = fields.Many2one('product.characteristics',
                                           string='Function of Product Step and Process Characteristics')
    ...
    man_line_ids = fields.One2many('process.suggestions', 'man_pro_id',
                                   string='Man Lines',

                                   )
    machine_line_ids = fields.One2many('process.suggestions', 'machine_pro_id',
                                       string='Machine Lines',
                                       )
    material_line_ids = fields.One2many('process.suggestions', 'material_pro_id',
                                        string='Material Lines',

                                        )
    environment_line_ids = fields.One2many('process.suggestions', 'environment_pro_id',
                                           string='Environment Lines',

                                           )
    method_line_ids = fields.One2many('process.suggestions', 'method_pro_id',
                                      string='Method Lines',
    ...
    def copy(self, default=None):
        ...
        new_suggestion = super(PfmeaSuggestions, self).copy(default)
...
# class PfmeaSuggestionsLines(models.Model):
#     _name = 'pfmea.suggestions.lines'
#     _description = 'Pfmea Suggestions Lines'
#
#     suggestion_id = fields.Many2one('pfmea.suggestions', string='Process Step', required=True, ondelete='cascade')
#
...
#
#
#     line_ids=fields.One2many('process.suggestions', 'line_id', string='Sub-Operations')
#
    
class ProcessSuggestions(models.Model):
    _name = 'process.suggestions'
    _description = 'Process Suggestions'

    man_pro_id = fields.Many2one('pfmea.suggestions', string='Man Process Reference')
    machine_pro_id = fields.Many2one('pfmea.suggestions', string='Machine Process Reference')
    material_pro_id = fields.Many2one('pfmea.suggestions', string='Material Process Reference')
    environment_pro_id = fields.Many2one('pfmea.suggestions', string='Environment Process Reference')
    method_pro_id = fields.Many2one('pfmea.suggestions', string='Method Process Reference')
