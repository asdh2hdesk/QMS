...
class ProcessReport(models.Model):
    _name = 'process.report'
    _description = 'PFMEA-VDA_AIAG'
    _inherit = ['iatf.sign.off.members', 'translation.mixin']

    company_name = fields.Many2one('res.company', string='Company Name',ondelete='cascade',default=lambda self: self.env.company.id)
    ...
    process_responsibility = fields.Char(string='Process Responsibility',translate=True)
    ...
    report_ids = fields.One2many('process.operations', 'operations_id', string='Operations')
    generate_xls_file = fields.Binary(string='Generated File')

    @api.model_create_multi
    def create(self, vals_list):
        records = super(ProcessReport, self).create(vals_list)
        ...
        for record in records:
            ...
            for operation in pfd_records.process_flow_line_ids.sorted(key="step"):
                for i in operation.process_op_lines_ids:
                    ...
                    if not suggestion:
                        suggestion = self.env['pfmea.suggestions'].create({
                            'process_step': operation.desc_of_operation,
                            'issue': i.element_no,
                            'station_no': operation.step,
                            'func_of_process_step': i.process_characteristics.id if i.process_characteristics else False,
                            'func_of_product_step': i.product_characteristics.id if i.product_characteristics else False,

                        })

                    self.env['process.operations'].create({
    ...
    def action_open_copy_wizard(self):
        ...
        return {
            ...
            'name': 'Copy Process Report Data',
            'res_model': 'process.report.copy.wizard',
            'view_mode': 'form',
            'target': 'new',
    ...
    def generate_xls_report(self):
        ...
        merge_cells = [
            ...
            'L9:M9',
              'F3:G3', 'F4:G4', 'F5:G5', 'F6:G6', 'H3:J3',
            'H4:J4', 'H5:J5', 'H6:J6',   'P3:R3',  'P4:R4',
            'P5:R5',  'P6:R6', 'C8:F8', 'G8:K8', 'L8:P8', 'Q8:W8', 'X8:AI8','F2:R2','S2:AJ7','C7:R7'
        ]
        ...
        for rec in self:
            ...
            for process_step_name, process_steps in process_sheets.items():
                ...
                for process_step in process_steps:
                    ...
                    func_process = process_step.func_of_process_step.name if process_step.func_of_process_step else ''
                    func_product = process_step.func_of_product_step.name if process_step.func_of_product_step else ''
                    ws[f'I{row}'] = f"{func_process} / {func_product}".strip(' /')
                ...
                if cur_row < row:
                    cur_row = row
                for rows in ws.iter_rows(min_row=1, max_row=cur_row, min_col=1, max_col=36):
                    for cell in rows:
...
class ProcessOperations(models.Model):
    _name = 'process.operations'
    _description = 'Process Operations'
    ...
    fpi_in_plant = fields.Char(string='In Plant',translate=True)
    fpi_ship_to_plant = fields.Char(string='Ship to Plant',translate=True)
    fpi_end_user = fields.Char(string='End User',translate=True)
    
    # Failure Effects (FE)
    fe_in_plant = fields.Char(string='In Plant')
    fe_customer_end = fields.Char(string='Customer End',translate=True)
    fe_end_user = fields.Char(string='End User',translate=True)
    
    
   
    
    station_no=fields.Char(string='Station No.')
    process_step = fields.Many2one('pfmea.suggestions',string='Process Step')
    process_step_name = fields.Char(related='process_step.process_step',string='Process Step',translate=True)
     # Function of the Process Step and Product Characteristics
    func_of_process_step = fields.Many2one('process.characteristics',
                                           string='Function of Process Step')
    func_of_product_step = fields.Many2one('product.characteristics',
                                           string='Product Characteristics')

    # fun_of_process_step = fields.Char(string='Function of Process Step and Product Characteristics')
    
     # Failure Mode (FM)
    failure_mode = fields.Char(string='Failure Mode (FM)',translate=True)
    
   
    
    
    # Special Product Characteristics
    special_product_characteristics = fields.Many2one('process.flow.class', string='Class')
    # process_ids = fields.One2many('process.sub.operations', 'sub_id', string='Sub-Operations')
    severity = fields.Integer(string='Severity (S) of FE', default=1)

    # Add SQL constraint for unique issue number
    _sql_constraints = [
        ('unique_issue_process_step', 'UNIQUE(project_id,process_step, issue)',
         'Issue number must be unique for a given Process Step!')
    ]
    man_line_ids = fields.One2many('process.sub.operations.lines', 'man_pro_id',
                                   string='Man Lines',

                                  )
    machine_line_ids = fields.One2many('process.sub.operations.lines', 'machine_pro_id',
                                       string='Machine Lines',
                                       )
    material_line_ids = fields.One2many('process.sub.operations.lines', 'material_pro_id',
                                        string='Material Lines',

                                       )
    environment_line_ids = fields.One2many('process.sub.operations.lines', 'environment_pro_id',
                                           string='Environment Lines',

                                          )
    method_line_ids = fields.One2many('process.sub.operations.lines', 'method_pro_id',
                                      string='Method Lines',
    ...
    # def _onchange_process_step_issue(self):
    ...
    #         ], limit=1)
    #
    #         if not suggestion and self.process_step:
    #             # If no specific match, use the process step
    ...
    #                 'fe_end_user', 'station_no', 'fun_of_process_step', 'failure_mode'
    #             ]
    #
    #             for field in fields_to_update:
    #                 self[field] = suggestion[field]
    #
    #             self.special_product_characteristics = suggestion.special_product_characteristics
    #
    #             # Clear existing sub-operations
    #             self.process_ids = [(5, 0, 0)]
    #
    #             # Create new sub-operations from suggestion lines
    #             new_sub_ops = []
    #             for suggestion_line in suggestion.suggestion_line_ids:
    #                 sub_op_vals = {
    #                     'work_type': suggestion_line.work_type,
    #                     'severity': suggestion_line.severity,
    #                     'line_ids': []
    #                 }
    #
    #                 line_items = []
    #                 for line in suggestion_line.line_ids:
    ...
    #                     }))
    #
    #                 sub_op_vals['line_ids'] = line_items
    #                 new_sub_ops.append((0, 0, sub_op_vals))
    #
    #             self.process_ids = new_sub_ops
    @api.onchange('process_step')
    ...
    def _update_fields_and_lines_from_suggestion(self, suggestion):
        ...
        fields_to_update = [
            'hca', 'process_item', 'fpi_process_item', 'fpi_in_plant',
            'fpi_ship_to_plant', 'fpi_end_user', 'fe_in_plant', 'fe_customer_end',
            'fe_end_user', 'severity', 'station_no', 'func_of_process_step', 'func_of_product_step','failure_mode'
        ]
        ...
        for record in self:
            ...
            suggestion_vals = {
                ...
                'station_no': record.station_no,
                 'func_of_process_step': record.func_of_process_step.id if record.func_of_process_step else False,
                'func_of_product_step': record.func_of_product_step.id if record.func_of_product_step else False,
            ...
            if suggestion:
                ...
                suggestion = self.env['pfmea.suggestions'].create(suggestion_vals)
    ...
    def write(self, vals):
        result = super(ProcessOperations, self).write(vals)
        self._sync_with_suggestion()
        return result

    @api.model
    def create(self, vals):
        record = super(ProcessOperations, self).create(vals)
        record._sync_with_suggestion()
        return record

    # def _sync_with_suggestion(self):
    #     for record in self:
    #         if record.process_step:
    #             # Prepare suggestion values
    #             suggestion_vals = {
    #                 'issue': record.issue,
    #                 'hca': record.hca,
    ...
    #             record.process_step.write(suggestion_vals)
    #
    #             # Clear existing suggestion lines
    #             record.process_step.suggestion_line_ids.unlink()
    #
    #             # Create new suggestion lines
    #             for sub_op in record.process_ids:
    #                 suggestion_line = self.env['pfmea.suggestions.lines'].create({
    #                     'suggestion_id': record.process_step.id,
    #                     'work_type': sub_op.work_type,
    #                     'severity': sub_op.severity,
    #                 })
    #
    #                 # Create line items for each sub-operation line
    #                 for line_item in sub_op.line_ids:
    #                     self.env['process.suggestions'].create({
    #                         'line_id': suggestion_line.id,
    #                         'process_work_element': line_item.process_work_element,
    #                         'function_of_process_work_element': line_item.function_of_process_work_element,
    #                         'failure_causes': line_item.failure_causes,
    #                         'current_prevention_control': line_item.current_prevention_control,
    #                         'occurrence': line_item.occurrence,
    #                         'current_detection_control': line_item.current_detection_control,
    #                         'detection': line_item.detection,
    #                         'fmea_ap': line_item.fmea_ap,
    #                         'filter_code': line_item.filter_code,
    #                         'prevention_action': line_item.prevention_action,
    #                         'detection_action': line_item.detection_action,
    #                         'responsible_person_name': line_item.responsible_person_name,
    #                         'target_completion_date': line_item.target_completion_date,
    #                         'status': line_item.status,
    #                         'action_taken': line_item.action_taken,
    #                         'completion_date': line_item.completion_date,
    #                         'severity_level': line_item.severity_level,
    #                         'occurrence_level': line_item.occurrence_level,
    #                         'detection_level': line_item.detection_level,
    #                         'special_characteristics': line_item.special_characteristics,
    #                         'pfmea_ap': line_item.pfmea_ap,
    #                         'remarks': line_item.remarks,
    #                     })
    #     return record


# class ProcessSubOperations(models.Model):
#     _name = 'process.sub.operations'
#     _description = 'Process Sub Operations'
#
#     sub_id = fields.Many2one('process.operations', string='Process Operations', ondelete='cascade')
#
#
...
#
#     severity = fields.Integer(string='Severity (S) of FE', default=1)
#     line_ids=fields.One2many('process.sub.operations.lines', 'sub_op_id', string='Sub-Operations-lines')
#
#     @api.constrains('work_type', 'sub_id')
#     def _check_unique_work_type_per_issue(self):
#         for record in self:
#             if record.work_type and record.sub_id and record.sub_id.issue:
#                 # Find all records with the same issue number and work type
#                 same_work_types = self.search([
#                     ('sub_id.process_step', '=', record.sub_id.process_step.id),
#                     ('sub_id.issue', '=', record.sub_id.issue),
...
#                         "The 4M Type '%s' already exists for issue number '%s' in process step '%s'. "
#                         "Each 4M Type must be unique within an issue for a specific process step."
#                     ) % (
#                                               dict(self._fields['work_type'].selection).get(record.work_type),
#                                               record.sub_id.issue,
#                                               record.sub_id.process_step.id  # Added missing argument
#                                           ))
#
#     @api.constrains('severity')
#     def _check_severity(self):
#         for record in self:
#             if record.severity and (record.severity < 1 or record.severity > 10):
#                 raise ValidationError("Severity must be between 1 and 10")
    
   
    
    
class ProcessSubOperationsLines(models.Model):
    _name = 'process.sub.operations.lines'
    _description = 'Process Sub Operations Lines'
    _inherit = "translation.mixin"
    


    man_pro_id = fields.Many2one('process.operations', string='Man Process Reference')
    machine_pro_id = fields.Many2one('process.operations', string='Machine Process Reference')
    material_pro_id = fields.Many2one('process.operations', string='Material Process Reference')
    environment_pro_id = fields.Many2one('process.operations', string='Environment Process Reference')
    method_pro_id = fields.Many2one('process.operations', string='Method Process Reference')
    ...
    function_of_process_work_element = fields.Text(string="Function of Process Work Element and Process Characteristics",translate=True)
    ...
    special_characteristics = fields.Text(string='Class')
...
class ProcessReportCopyWizard(models.TransientModel):
    _name = 'process.report.copy.wizard'
    _description = 'Copy Process Report Data'

    current_record_id = fields.Many2one('process.report', string='Current Report', required=True)
    selected_record_id = fields.Many2one('process.report', string='Copy From', required=True)

    def action_copy_data(self):
        self.ensure_one()
        current = self.current_record_id
        source = self.selected_record_id

        if not source.report_ids:
            raise ValidationError(_("Selected report has no process operations to copy."))

        # Copy high-level fields
        current.write({
            'myp': source.myp,
            'pfmea_start_date': source.pfmea_start_date,
            'pfmea_revision_date': source.pfmea_revision_date,
            'pfmea_aid_no': source.pfmea_aid_no,
            'process_responsibility': source.process_responsibility,
            'confidentiality_level': source.confidentiality_level,
        })

        # Clear and copy report_ids (One2many -> process.operations)
        current.report_ids = [(5, 0, 0)]
        new_ops = []

        for op in source.report_ids:
            new_op_vals = {
                'issue': op.issue,
                'hca': op.hca,
                'process_item': op.process_item,
                'fpi_process_item': op.fpi_process_item,
                'fpi_in_plant': op.fpi_in_plant,
                'fpi_ship_to_plant': op.fpi_ship_to_plant,
                'fpi_end_user': op.fpi_end_user,
                'fe_in_plant': op.fe_in_plant,
                'fe_customer_end': op.fe_customer_end,
                'fe_end_user': op.fe_end_user,
                'station_no': op.station_no,
                'process_step': op.process_step.id,
                'func_of_process_step': op.func_of_process_step.id if op.func_of_process_step else False,
                'func_of_product_step': op.func_of_product_step.id if op.func_of_product_step else False,
                'failure_mode': op.failure_mode,
                'special_product_characteristics': op.special_product_characteristics.id if op.special_product_characteristics else False,
                'severity': op.severity,
            }

            # Collect sub-lines
            def get_sub_lines(lines):
                return [(0, 0, {
                    'work_type': l.work_type,
                    'process_work_element': l.process_work_element,
                    'function_of_process_work_element': l.function_of_process_work_element,
                    'failure_causes': l.failure_causes,
                    'current_prevention_control': l.current_prevention_control,
                    'occurrence': l.occurrence,
                    'current_detection_control': l.current_detection_control,
                    'detection': l.detection,
                    'filter_code': l.filter_code,
                    'prevention_action': l.prevention_action,
                    'detection_action': l.detection_action,
                    'responsible_person_name': l.responsible_person_name.id if l.responsible_person_name else False,
                    'target_completion_date': l.target_completion_date,
                    'status': l.status,
                    'action_taken': l.action_taken,
                    'completion_date': l.completion_date,
                    'severity_level': l.severity_level,
                    'occurrence_level': l.occurrence_level,
                    'detection_level': l.detection_level,
                    'special_characteristics': l.special_characteristics,
                    'pfmea_ap': l.pfmea_ap,
                    'remarks': l.remarks,
                }) for l in lines]

            new_op_vals.update({
                'man_line_ids': get_sub_lines(op.man_line_ids),
                'machine_line_ids': get_sub_lines(op.machine_line_ids),
                'material_line_ids': get_sub_lines(op.material_line_ids),
                'environment_line_ids': get_sub_lines(op.environment_line_ids),
                'method_line_ids': get_sub_lines(op.method_line_ids),
            })

            new_ops.append((0, 0, new_op_vals))

        current.report_ids = new_ops
