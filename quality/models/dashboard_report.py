...
class PFMEADashboard(models.Model):
    ...
    issue = fields.Char(string='Issue #', readonly=True,translate=True)
    process_step_name = fields.Char(string='Process Step', readonly=True,translate=True)
    prevention_action = fields.Text(string='Prevention Action', readonly=True,translate=True)
    detection_action = fields.Text(string='Detection Action', readonly=True,translate=True)
...
class PFMEADashboardScheduler(models.Model):
    ...
    def refresh_dashboard(self):
        self.env['dashboard.report'].refresh_dashboard_data()
        return True


# Add hooks to update dashboard when process records change
class ProcessOperations(models.Model):
    _inherit = 'process.operations'

    @api.model_create_multi
    def create(self, vals_list):
        records = super(ProcessOperations, self).create(vals_list)
        self.env['dashboard.report'].refresh_dashboard_data()
        return records
    ...
    def unlink(self):
        result = super(ProcessOperations, self).unlink()
        self.env['dashboard.report'].refresh_dashboard_data()
        return result
...
class ProcessSubOperationsLines(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        records = super(ProcessSubOperationsLines, self).create(vals_list)
        self.env['dashboard.report'].refresh_dashboard_data()
        return records

    def write(self, vals):
        result = super(ProcessSubOperationsLines, self).write(vals)
        self.env['dashboard.report'].refresh_dashboard_data()
        return result

    def unlink(self):
        result = super(ProcessSubOperationsLines, self).unlink()
        self.env['dashboard.report'].refresh_dashboard_data()
        return result
