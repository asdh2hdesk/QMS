...
class IncomingIR(models.Model):
    _name = 'in.coming.inspection.report'
    ...
    line_ids = fields.One2many('in.coming.inspection.report.line', 'inspection_id', string="Inspection Lines")
    ...
    def _sync_to_control_plan(self):
        """Sync updates from Incoming IR to Control Plan."""
        for rec in self:
    ...
    #     records = super(IncomingIR, self).create(vals_list)
    #
    #     for record in records:
    #         cp_records = self.env["control.plan"].search([
    #             ('project_id', '=', record.project_id.id),
    #             ('final_status', '=', 'approved')
    #         ])
    #
    #         for operation in cp_records.process_line_ids.sorted(key="process_step"):
    #             # Update the current record instead of creating a new one
    #             record.write({
    #                 'op_no': operation.process_step,
    #                 'op_name': operation.process_name,
    #             })
    #
    #             # Create inspection lines for the current record
    #             for i in operation.process_char_ids:
    #                 inspected_by_id = i.method_inspected_by.id if i.method_inspected_by else False
    #                 symbol_id = operation.char_class.id if operation.char_class else False
    #
    #                 self.env['in.coming.inspection.report.line'].create({
    #                     'inspection_id': record.id,
    #                     'ele_no': i.char_no,
    #                     'dimension_des': i.method_description.id if i.method_description else False,
    #                     'dimension_spec': i.method_product_spec,
    #                     'inspection_method': i.method_evaluation.id if i.method_evaluation.gauge else False,
    #                     'inspected_by': inspected_by_id,
    #                     'symbol': symbol_id,  # Using the ID instead of the object
    #                 })
    #
    #     return records
    ...
    def generate_xls_report(self):
        ...
        # Headers dictionary
        headers = {
            'C1':'In Coming Inspection Report',
        ...
        attachment = self.env['ir.attachment'].create({
            'name': 'INCOMING_INSPECTION.xlsx',
            'type': 'binary',
            'datas': base64.b64encode(output.getvalue()),
            'res_model': 'in.coming.inspection.report',
            'res_id': self.id,
...
class IncomingIRLine(models.Model):
    _name = 'in.coming.inspection.report.line'
    _description = 'Report of Incoming Inspection Line'
    _order = 'sequence'
    _inherit = "translation.mixin"

    inspection_id = fields.Many2one('in.coming.inspection.report', string='Inspection Report', ondelete='cascade')
    ...
    observations = fields.One2many('inspection.observation', 'inspection_line_id', string="Observations")
    ...
    @api.model
    def create(self, vals):
        res = super(IncomingIRLine, self).create(vals)
    ...
    def _compute_sr_no(self):
        for record in self:
            record.sr_no = record.sequence

    def _resequence_records(self, inspection_id):
        if not inspection_id:
            return

        lines = self.search([('inspection_id', '=', inspection_id.id)], order="sequence, id")
...
class InspectionObservation(models.Model):
    _name = 'inspection.observation'
    _description = 'Inspection Observations'
    _inherit = "translation.mixin"

    inspection_line_id = fields.Many2one('in.coming.inspection.report.line', string="Inspection Line", ondelete="cascade")
